window.addEventListener('load',()=>{setTimeout(()=>document.getElementById('loader').classList.add('hidden'),2000)});
        ...
        document.querySelectorAll('a,button,.feature-card,.pricing-card,.ov-feature').forEach(el=>{el.addEventListener('mouseenter',()=>ring.classList.add('big'));el.addEventListener('mouseleave',()=>ring.classList.remove('big'))});
        ...
        class P{constructor(){this.reset()}reset(){this.x=Math.random()*canvas.width;this.y=canvas.height+10;this.s=Math.random()*2+0.5;this.v=Math.random()*0.7+0.2;this.o=Math.random()*0.5+0.1;this.d=(Math.random()-0.5)*0.3}update(){this.y-=this.v;this.x+=this.d;if(this.y<-10)this.reset()}draw(){ctx.save();ctx.globalAlpha=this.o;ctx.fillStyle='#ff0000';ctx.shadowColor='#ff0000';ctx.shadowBlur=7;ctx.beginPath();ctx.arc(this.x,this.y,this.s,0,Math.PI*2);ctx.fill();ctx.restore()}}
        ...
        new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting)e.target.classList.add('active')}),{rootMargin:'0px 0px -60px 0px',threshold:0.08}).observe&&allR.forEach(el=>new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting)e.target.classList.add('active')}),{rootMargin:'0px 0px -60px 0px',threshold:0.08}).observe(el));
